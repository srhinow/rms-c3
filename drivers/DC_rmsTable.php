 ...
 * @package    Core
 */
class DC_rmsTable extends \DataContainer implements \listable, \editable
{
	...
	* @param boolean
	*/
	protected $overwriteRmsData = false;

	/**
	* contain no-released data
	* @param array
	*/
	protected $rmsArr = array();
...
<a href="'.$this->getReferer(true).'" class="header_back" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['backBTTitle']).'" accesskey="b" onclick="Backend.getScrollOffset()">'.$GLOBALS['TL_LANG']['MSC']['backBT'].'</a>
	...
	public function getPalette()
	{
		...
		if (!empty($GLOBALS['TL_DCA'][$this->strTable]['palettes']['__selector__']))
		{
			...
			if($this->overwriteRmsData && count($this->rmsArr) > 0)
			{
				reset($this->rmsArr);
				foreach($this->rmsArr as $field =>$value) $objFields->{$field} = $value;
			}
