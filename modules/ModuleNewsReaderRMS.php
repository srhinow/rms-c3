 ...
 * @package    News
 */
class ModuleNewsReaderRMS extends \ModuleNews
{
	...
	protected function compile()
	{
		...
		//overwrite with unreleased content
		if(\Input::get('do') == 'preview')
		{
			$rmsObj = $this->Database->prepare("SELECT `data` FROM `tl_rms` WHERE `ref_id`=? AND `ref_table`=?")
									 ->limit(1)
									 ->execute($objArticle->id, 'tl_news');

			if($rmsObj->numRows > 0)
			{
				$rmsArr = unserialize($rmsObj->data);

				foreach($rmsArr as $field => $value)
				{
					$objArticle->{$field} = $value;
				}
			}
		}
